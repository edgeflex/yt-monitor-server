...
async function fetchChannelFeed(channelId) {
  ...
  if (!res.ok) {
    if (res.status === 404) {
      /* YouTube RSS endpoint has known intermittent 404s — not a code error */
      return [];
    }
    throw new Error(`RSS feed returned HTTP ${res.status} for channel ${channelId}`);
  }
...
app.post('/yt-sync', async (req, res) => {
...
────────────────────────────────────────────────────────────────────────── */
app.post('/rss-proxy', async (req, res) => { /* No auth — public RSS feeds only */
  const { url } = req.body || {};
