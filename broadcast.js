...
/* fetch polyfill — Node 18+ has native fetch; older versions use node-fetch */
const _fetchImpl = typeof fetch !== 'undefined'
  ? fetch
...
Write the complete dual-anchor script now. Return only valid JSON.`;
  ...
  const res = await _fetchImpl(`${GROQ_BASE}/chat/completions`, {
    method: 'POST',
    headers: {
...
async function synthesiseLine(text, anchor) {
  ...
  const res = await _fetchImpl(`${EL_BASE}/text-to-speech/${voiceId}`, {
    method: 'POST',
    headers: {
      'Content-Type':  'application/json',
